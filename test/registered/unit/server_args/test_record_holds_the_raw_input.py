...
This is the whole contract in one sentence, and until now nothing asserted it:
the other guards each pin one channel -- no bare assignment, the stash explains
every moved field, no reader takes a decision off the record -- while this one
compares the finished record against the snapshot taken before any handler ran.
It fails on a channel nobody thought to pin: a `setattr` inside a new handler, a
pass slot that writes through, a nested object edited under the record.

Two directions matter. A field must not be *rebound* (the snapshot holds the
value the caller passed, and `getattr` must still answer with it), and an
...
# One shape per family of handlers that decides something: the topology passes,
# the speculative pass, the cuda-graph parse, the memory defaults, the attention
# backend fallbacks, and the LoRA normalization (which rewrites its input into
# `LoRARef` objects and so is the most likely to write a field back).
_SHAPES = {
    "plain": {},
...
class TestRecordHoldsTheRawInput(CustomTestCase):
    def setUp(self):
        # Resolution writes environment variables, and those outlive the record
        # that set them.
        super().setUp()
    ...
    def _resolve(self, **supplied):
        ...
        # The launch validates after resolving, and validation is where the
        # LoRA normalization rewrites its input into `LoRARef` objects and where
        # `_hisparse_validation` runs -- the writes most likely to land on a
        # field. Running it here is what makes those shapes cover the claim.
    ...
    def test_the_snapshot_is_the_value_the_caller_passed(self):
        # The other direction: a snapshot taken *after* a handler rewrote the
        # field would agree with the record and hide the write.
