...
class TestGrpcServerArgs(CustomTestCase):
    ...
    def test_start_server_call_site_matches_native_signature(self):
        ...
        # Host and worker count both come from the `serving` bag; the dummy
        # boundary skips `_handle_deprecated_args`, so the count is supplied
        ...
        # The native binding takes `worker_threads: usize`; the bag's value has
        # to arrive as one, which asserting only the kwarg set would not catch.
